...
```bash
# 出力ディレクトリを作成
mkdir -p blog-output/images blog-output/videos blog-output/gifs
...
```bash
# ページ全体
playwright-cli -s blog screenshot --filename blog-output/images/{name}.png --full-page

# 特定要素のスクリーンショット
playwright-cli -s blog screenshot {element-ref} --filename blog-output/images/{name}.png
```
...
```bash
# 録画開始
playwright-cli -s blog video-start --filename blog-output/videos/{name}.webm
...
```
blog-output/YYYY-MM-DD-{theme-slug}-base.md
```

例: `blog-output/2026-04-07-kuzu-wasm-browser-graphdb-base.md`
