---
name: dogfood
description: Systematically explore and test a web application to find bugs, UX issues, and other problems using Playwright CLI (@playwright/cli). Use when asked to "dogfood", "QA", "exploratory test", "find issues", "bug hunt", "test this app/site/platform", or review the quality of a web application. Produces a structured report with full reproduction evidence -- step-by-step screenshots, repro videos, and detailed repro steps for every issue -- so findings can be handed directly to the responsible teams.
allowed-tools: Bash(playwright-cli:*), Bash(npx playwright-cli:*), Bash(pnpm exec playwright-cli:*)
---
...
| **Session name** | Slugified domain (e.g., `vercel.com` -> `vercel-com`) | `-s my-session` |
...
Always use `playwright-cli` directly -- never `npx playwright-cli`. The direct binary is faster. Use `npx` only if the direct binary is not found.
